...
- When direction is automatic and the input contains only strong RTL codepoints plus neutral and weak numeric characters, shaped glyphs are mirrored into RTL visual order after positioning while preserving the internal order of ASCII and Arabic-Indic formatted numeric runs.
- Mixed strong LTR/RTL text reverses RTL runs inside predominantly LTR text and preserves internal formatted numeric run order inside those RTL runs.
