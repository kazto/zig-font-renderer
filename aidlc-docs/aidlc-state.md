...
- **Current Stage**: CONSTRUCTION - Font cmap Module Split Review
...
- **Current Stage**: CONSTRUCTION - Font cmap Module Split Review
- **Last Completed**: Unit 1: Font cmap Module Split
- **Next Stage**: Unit 2: Remaining complex script reordering or Unit 3: Remaining Type 2 Operators/CFF2
- **Status**: The parser accepts standalone sfnt fonts and TTC collections by loading the first face, enabling common CJK `.ttc` fonts through the existing API. Font cmap selection and format 4/12 lookup now live in `font_cmap.zig`, with shared parser public types in `font_types.zig`, while `font_parser.zig` remains the public Face facade. Shaping supports basic cmap shaping, coarse automatic script/language tag inference, conservative default feature policy, RTL-only visual ordering, mixed RTL run reordering inside predominantly LTR text, explicit top-to-bottom vertical layout, optional vertical metric lookup from `vhea`/`vmtx`, optional vertical origin lookup from `VORG`, legacy kern fallback, GSUB/GPOS lookup handling (Single, Alternate, Ligature, Contextual Formats 1/2/3, Chained Contextual Formats 1/2/3, Single/Pair Adjustment, Mark-to-Base, Mark-to-Ligature with cluster-based component selection, and Mark-to-Mark) at the tested scope, contextual sub-lookups that change glyph stream length, checked OpenType Layout offsets, and caller-provided selection options; SVG output supports TrueType and CFF outlines with styling, transforms, positioned glyph offsets from shaping, transformed composite glyphs, vertical-origin-aware glyph transforms, and point-matched TrueType composite components at the tested scope; CFF support includes subroutines, flex, calculation operators, CID-keyed local subroutine resolution, stem3 hint counting for mask alignment, Type 2 `setcurrentpoint`, `closepath`, and othersubr/pop compatibility; initial CFF2 support can parse CFF2 INDEX/Top DICT/CharStrings/FDArray/FDSelect/Private Subrs, read Variation Store region counts, route charstrings through SVG outline rendering, and render `blend` at the default instance by ignoring variation deltas. Source organization now keeps `shaper.zig` as a smaller public facade with feature policy and direction helpers split out, moves Type 2 stack/calculation compatibility operators into `type2_stack_ops.zig`, moves Type 2 path/flex emission helpers into `type2_path_ops.zig`, and keeps `svg_renderer.zig` as a small public SVG document facade while glyph outline parsing, bounds, transforms, and composite expansion live in `svg_glyph.zig`. Selecting non-first TTC faces, full Unicode Bidi, complex script reordering, CFF2 non-default variation interpolation, and remaining full typography coverage remain pending.
